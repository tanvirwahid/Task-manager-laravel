...
<div class="max-w-md w-full max-h-fit bg-white p-8 rounded-lg">
    <form method="POST" action="{{route('tasks.update', ['task' => $task->id])}}">
        ...
        <div class="mb-4">
            ...
            @can('manage')
            <input id="name" type="text" name="name" class="border-2 border-gray-600 rounded-lg p-2 w-full"
    ...
    <form method="POST" action="{{route('tasks.assign', ['id' => $task->id])}}">
        ...
        <select id="user_ids" name="user_ids[]"
                class="select2-multiple form-control border-2 border-gray-600 rounded max-h-10 mb-2.5"
                multiple="multiple">
            @foreach($users as $user)
            <option value="{{ $user->id }}" {{ $task->users->contains($user->id) ? 'selected' : '' }}>{{ $user->name
                }}
            </option>
            @endforeach
        </select>
        @else
        <select id="user_ids" name="user_ids[]" disabled
                class="select2-multiple form-control border-2 border-gray-600 rounded max-h-10 mb-2.5"
                multiple="multiple">
            @foreach($users as $user)
            <option value="{{ $user->id }}" {{ $task->users->contains($user->id) ? 'selected' : '' }}>{{ $user->name
        ...
        @can('manage')
        <div class="mb-6">
            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 mt-1.5">Save
            </button>
        </div>
        @endcan
    </form>
